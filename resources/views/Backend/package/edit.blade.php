            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$package->name}}</span>
                    </div>
                </li>
            </ol>
        ...
        <form id="package-form" action="{{ route('admin.package.update', $package->id) }}" method="POST">
            ...
            <div class="flex flex-col lg:!flex-row gap-6">
                <div class="w-full lg:!w-2/3">
                    <div class="bg-white p-6 rounded-lg shadow">
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                <label for="subtitle" class="block text-sm font-medium text-gray-700 mb-2">Package Subtitle <span class="text-red-600">*</span></label>
                                <textarea
                                    id="subtitle" name="subtitle" required rows="3"
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                                    placeholder="Enter package subtitle"
                                >{{ old('subtitle', $package->subtitle) }}</textarea>
                                @error('subtitle')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="is_best_deal" id="is_best_deal" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
                                    <option value="0" {{ old('is_best_deal', $package->is_best_deal) == 0 ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ old('is_best_deal', $package->is_best_deal) == 1 ? 'selected' : '' }}>Yes</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="w-full lg:!w-1/3">
                    <div class="bg-white p-6 rounded-lg shadow">
                        <div class="grid grid-cols-1 gap-6">
                            <h5 class="block text-sm font-medium text-gray-700">Package Features</h5>
                            ...
                            <button type="button" id="add-feature" class="text-teal-600 flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 极 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-1">
