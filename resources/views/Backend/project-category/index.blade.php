...
@section('contents')
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">All Project Categories</span>
                        </div>
                    </li>
                </ol>
            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm text-left text-gray-600">
                                <thead class="bg-gray-100 text-gray-700 uppercase text-xs">
                                    <tr>
                                        <th class="px-14 py-3 w-3/5">Name</th>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @forelse($categories as $category)
                                        <tr>
                                            <td class="px-14 py-4">
                                                <div class="max-w-2xl truncate" title="{{ $category->name }}">
                                                    {{ $category->name }}
                                                </div>
                                            </td>
                                            <td class="px-14 py-4 flex justify-end gap-x-2">
                                                <a href="{{ route('admin.project-category.edit', $category->id) }}" class="text-blue-500 hover:text-blue-700 p-1 rounded hover:bg-blue-50">
                                                ...
                                                <button class="text-red-500 hover:text-red-700 p-1 rounded hover:bg-red-50" type="button" onclick="confirmDelete({{ $category->id }})">
                                        ...
                                        <tr>
                                            <td colspan="2" class="px-6 py-4 text-center font-medium text-gray-700">
                                                <div class="flex flex-col items-center justify-center gap-x-4 py-2">
                                                    ...
                                                    <span class="pt-2 text-lg">No Project Category found.</span>
                                                </div>
                                                <span class="text-gray-500">Create your first project category to get started!</span>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                ...
                <div class="w-full lg:!w-1/3">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl font-semibold mb-4 text-gray-800 border-b pb-2">
                            {{ isset($categoryToEdit) ? 'Edit Project Category' : 'Add New Project Category' }}
                        </h2>

                        <form action="{{ isset($categoryToEdit) ? route('admin.project-category.update', $categoryToEdit->id) : route('admin.project-category.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-4">
                                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Name <span class="text-red-600 font-bold">*</span></label>
                                <input type="text" id="name" name="name" value="{{ old('name', $categoryToEdit->name ?? '') }}"
                            ...
                            <div class="flex justify-end mt-6">
                                @if(isset($categoryToEdit))
                                    <a href="{{ route('admin.project-category.all') }}" class="px-4 py-2 border !border-gray-300 rounded-lg text-gray-700 hover:!bg-gray-50 mr-2">
                                        Cancel
                                    </a>
                                @endif
                                <button type="submit" class="px-4 py-2 !bg-teal-600 text-white rounded-lg hover:!bg-teal-700">
                                    {{ isset($categoryToEdit) ? 'Update' : 'Create' }} Project Category
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <div id="delete-modal" class="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <p class="text-gray-600 mb-6">Are you sure you want to delete this project category? This action cannot be undone.</p>
...
@push('scripts')
    <script>
        function confirmDelete(id) {
            ...
            form.action = `/admin/project-category/${id}`;
