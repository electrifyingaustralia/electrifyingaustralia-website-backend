                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($blog->media)
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($blog->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                ...
                                <div id="selected-logo-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $blog->media ? '' : 'hidden' }}">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            @if($blog->media->mime_type && str_starts_with($blog->media->mime_type, 'image/'))
                                                <img id="selected-logo-preview" src="{{ $blog->media->url }}" alt="Selected media" class="w-12 h-12 object-cover rounded">
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($blog->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
