...
<div class="flex-1 p-6">
    <div class="max-w-5xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$admin->name}}</span>
                    </div>
                </li>
            </ol>
            <a href="{{ route('admin.users.all') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
        ...
        <form id="admin-form" action="{{ route('admin.users.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($admin->media)
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($admin->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                    ...
                                    <div class="flex flex-col justify-center gap-2">
                                        ...
                                        <input type="hidden" id="selected-media-id" name="media_id" value="{{ old('media_id', $admin->media_id) }}">
                                    </div>
                                </div>
                                ...
                                <div id="selected-logo-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $admin->media ? '' : 'hidden' }}">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            @if($admin->media->mime_type && str_starts_with($admin->media->mime_type, 'image/'))
                                                <img id="selected-logo-preview" src="{{ $admin->media->url }}" alt="Selected media" class="w-12 h-12 object-cover rounded">
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($admin->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                            ...
                                            @endif
                                            <div>
                                                <p id="selected-logo-name" class="text-sm font-medium">{{ $admin->media->original_name }}</p>
                                                <p id="selected-logo-size" class="text-xs text-gray-500">{{ formatFileSize($admin->media->file_size) }}</p>
                                            </div>
                                        </div>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = response.redirect || '{{ route('admin.users.all') }}';
            } else {
