...
<div class="flex-1 p-6">
    <div class="max-w-5xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">Create New Admin</span>
                    </div>
                </li>
            </ol>
        ...
        <form id="admin-form" action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    localStorage.setItem('toastr_success', response.message) || '{{ route('admin.users.all') }}';

                    // Redirect to index page
                    window.location.href = response.redirect;
                } else {
                    toastr.error(response.message || 'An error occurred');
                    $('button[type="submit"]').prop('disabled', false).text('Create Admin');
                }
            },
            error: function(xhr) {
                // Handle errors
