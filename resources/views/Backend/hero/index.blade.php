...
<div class="flex-1 p-3">
    <div class="max-w-[90rem] mx-auto">
        <div class="flex justify-between items-center mb-5" aria-label="Breadcrumb">
            ...
            <a href="{{ route('admin.hero.create') }}" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg">
        ...
        <div class="bg-white p-4 rounded-lg shadow mb-6">
            <form action="{{ route('admin.hero.all') }}" method="GET">
                <div class="flex gap-4">
                    <div class="flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search heroes..." class="w-full p-2 border !border-gray-300 rounded-lg">
                    </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($heroes as $hero)
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $hero->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $hero->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-8 flex gap-x-2">
                                <a href="{{ route('admin.hero.edit', $hero->id) }}" class="text-blue-500 hover:text-blue-700">
                                ...
                                <button class="text-red-500 hover:text-red-700" type="button" onclick="confirmDelete({{ $hero->id }})">
...
<div id="delete-modal" class="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white p-6 rounded-lg w-full max-w-md">
        ...
        <p class="text-gray-600 mb-6">Are you sure you want to delete this hero section? This action cannot be undone.</p>
