        ...
        <div class="flex justify-between items-center mb-5" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">Edit Hero File</span>
                    </div>
                </li>
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$hero->title}}</span>
                    </div>
                </li>
            </ol>
            <a href="{{ route('admin.hero.all') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
                <div class="flex items-center gap-x-2">
                    ...
                    <span>Back to All Hero Files</span>
                </div>
            </a>
        </div>

        <form id="hero-form" action="{{ route('admin.hero.update', $hero->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active"
                                    ...
                                    <option value="1" {{ old('is_active', $hero->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('is_active', $hero->is_active) == 0 ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($hero->media)
                                            @if($hero->media->mime_type && str_starts_with($hero->media->mime_type, 'image/'))
                                                <img src="{{ $hero->media->url }}" alt="{{ $hero->title }}" class="w-full h-full object-cover rounded-lg">
                                            @elseif($hero->media->mime_type && str_starts_with($hero->media->mime_type, 'video/'))
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($hero->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                    ...
                                    <div class="flex flex-col justify-center gap-2">
                                        ...
                                        <input type="hidden" id="selected-media-id" name="media_id" value="{{ old('media_id', $hero->media_id) }}">
                                    </div>
                                </div>

                                <!-- Selected Logo Info -->
                                <div id="selected-logo-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $hero->media ? '' : 'hidden' }}">
                                    @if($hero->media)
...
$(document).ready(function() {
    ...
    selectedMedia = {
        id: {{ $hero->media_id }},
        url: '{{ $hero->media->url }}',
        name: '{{ $hero->media->original_name }}',
        size: {{ $hero->media->file_size }},
        file: null,
        type: 'library'
    };
    @endif
    ...
    function applySelectedMedia() {
        ...
        if (selectedMedia.url.match(/\.(jpg|jpeg|png|gif|svg|webp)$/i) ||
            ...
            switch(fileExtension) {
                ...
                case 'docx':
                    bgColor = 'bg-blue-100';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 极 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                    break;
                case 'xls':
                case 'xlsx':
                    bgColor = 'bg-green-100';
                    iconSvg = '<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 极 3v18"/>';
                    break;
                default:
                    bgColor = 'bg-gray-200';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2极 16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/>';
            }
        ...
        if (selectedMedia.url.match(/\.(jpg|jpeg|png|gif|svg|webp)$/i) ||
            (selectedMedia.mime_type && selectedMedia.mime_type.startsWith('image/'))) {
            $('#selected-logo-info .flex.items-center.space-x-3 > div:first-child').html(`<img id="selected-logo-preview" src="${selectedMedia.url}" alt="Selected media" class="w-12 h-12 object-cover rounded">`);
        } else {
            let bgColor = 'bg-gray-200';
            let iconSvg = '';
            ...
            switch(fileExtension) {
                case 'pdf':
                    bgColor = 'bg-red-100';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2极 4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                    break;
                case 'doc':
                case 'docx':
                    bgColor = 'bg-blue-100';
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = response.redirect || '{{ route('admin.hero.all') }}';
            } else {
                toastr.error(response.message || 'An error occurred');
                $('button[type="submit"]').prop('disabled', false).text('Update Hero');
            }
        },
        error: function(xhr) {
