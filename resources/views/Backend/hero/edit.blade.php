...
@section('contents')
    ...
    <div class="flex-1 p-6">
        <div class="max-w-5xl mx-auto">
            ...
            <form id="hero-form" action="{{ route('admin.hero.update', $hero->id) }}" method="POST"
                    ...
                    <div class="w-full">
                        ...
                        <div class="bg-white p-6 rounded-lg shadow">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                ...
                                <div>
                                    <label for="is_active" class="block text-sm font-medium text-gray-700 mb-1">Hero
                                        Status</label>
                                    <select name="is_active" id="is_active"
                                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">

                                        <option value="1"
                                            {{ old('is_active', $hero->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="0"
                                            {{ old('is_active', $hero->is_active) == 0 ? 'selected' : '' }}>Inactive
                                        </option>
                                    </select>
                                </div>

                                <div>
                                    <label for="video_url" class="block text-sm font-medium text-gray-700 mb-2">Hero Video
                                        Url</label>
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-4">
                                        ...
                                        <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50"
                                            ...
                                            @if ($hero->media)
                                                @if ($hero->media->mime_type && str_starts_with($hero->media->mime_type, 'image/'))
                                                    <img src="{{ $hero->media->url }}" alt="{{ $hero->title }}"
                                        ...
                                        <div class="flex flex-col justify-center gap-2">
                                            ...
                                            <input type="hidden" id="selected-media-id" name="media_id"
                                                value="{{ old('media_id', $hero->media_id) }}">
                                        </div>
                                    </div>

                                    <!-- Selected Logo Info -->
                                    <div id="selected-logo-info"
                                        class="mt-3 p-3 bg-gray-50 rounded-lg {{ $hero->media ? '' : 'hidden' }}">
                                        @if ($hero->media)
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center space-x-3">
                                                    @if ($hero->media->mime_type && str_starts_with($hero->media->mime_type, 'image/'))
                                                        <img id="selected-logo-preview" src="{{ $hero->media->url }}"
                                                    ...
                                                    <div>
                                                        <p id="selected-logo-name" class="text-sm font-medium">
                                                            {{ $hero->media->original_name }}</p>
                                                        <p id="selected-logo-size" class="text-xs text-gray-500">
                                                            {{ formatFileSize($hero->media->file_size) }}</p>
                                                    </div>
                                                </div>
            ...
            `);
                    ...
                    switch (fileExtension) {
                        ...
                        case 'docx':
                            bgColor = 'bg-blue-100';
                            iconSvg =
                                '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 极 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                            break;
                        case 'xls':
                        case 'xlsx':
                            bgColor = 'bg-green-100';
                            iconSvg =
                                '<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 极 3v18"/>';
                            break;
                        default:
                            bgColor = 'bg-gray-200';
                            iconSvg =
                                '<path d="M15 2H6a2 2 0 0 0-2 2极 16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/>';
                    }
                ...
                if (selectedMedia.url.match(/\.(jpg|jpeg|png|gif|svg|webp)$/i) ||
                    ...
                    switch (fileExtension) {
                        case 'pdf':
                            bgColor = 'bg-red-100';
                            iconSvg =
                                '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2极 4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                            break;
                        case 'doc':
                        case 'docx':
                            bgColor = 'bg-blue-100';
