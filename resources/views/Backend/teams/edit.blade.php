...
<div class="flex-1 p-6">
    <div class="max-w-5xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">Edit Team Member</span>
                    </div>
                </li>
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$team->name}}</span>
                    </div>
                </li>
            </ol>
        ...
        <form id="team-form" action="{{ route('admin.teams.update', $team->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                            ...
                            <div>
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($team->media)
                                            @if($team->media->mime_type && str_starts_with($team->media->mime_type, 'image/'))
                                                <img src="{{ $team->media->url }}" alt="{{ $team->title }}" class="w-full h-full object-cover rounded-lg">
                                            @elseif($team->media->mime_type && str_starts_with($team->media->mime_type, 'video/'))
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($team->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                    ...
                                    <div class="flex flex-col justify-center gap-2">
                                        ...
                                        <input type="hidden" id="selected-media-id" name="media_id" value="{{ old('media_id', $team->media_id) }}">
                                    </div>
                                </div>

                                <!-- Selected Media Info -->
                                <div id="selected-logo-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $team->media ? '' : 'hidden' }}">
                                    @if($team->media)
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-3">
                                            @if($team->media->mime_type && str_starts_with($team->media->mime_type, 'image/'))
                                                <img id="selected-logo-preview" src="{{ $team->media->url }}" alt="Selected media" class="w-12 h-12 object-cover rounded">
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($team->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                            ...
                                            @endif
                                            <div>
                                                <p id="selected-logo-name" class="text-sm font-medium">{{ $team->media->original_name }}</p>
                                                <p id="selected-logo-size" class="text-xs text-gray-500">{{ formatFileSize($team->media->file_size) }}</p>
                                            </div>
                                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Enter team member facebook link"
                                />
                                @error('facebook_link')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Enter team member twitter link"
                                />
                                @error('twitter_link')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Enter team member linkedin link"
                                />
                                @error('linkedin_link')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                                    value="{{old('pinterest_link', $team->pinterest_link)}}"
                                    placeholder="Enter team member pinterest link"
                                />
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                                    value="{{old('youtube_link', $team->youtube_link)}}"
                                    placeholder="Enter team member youtube link"
                                />
                                @error('youtube_link')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
...
$(document).ready(function() {
    ...
    selectedMedia = {
        id: {{ $team->media_id }},
        url: '{{ $team->media->url }}',
        name: '{{ $team->media->original_name }}',
        size: {{ $team->media->file_size }},
        file: null,
        type: 'library'
    };
    @endif
    ...
    function applySelectedMedia() {
        ...
        if (selectedMedia.url.match(/\.(jpg|jpeg|png|gif|svg|webp)$/i) ||
            ...
            switch(fileExtension) {
                case 'pdf':
                    bgColor = 'bg-red-100';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2极 4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                    break;
                case 'doc':
                case 'docx':
                    bgColor = 'bg-blue-100';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v极 4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                    break;
                case 'xls':
                case 'xlsx':
                    bgColor = 'bg-green-100';
                    iconSvg = '<rect width="18" height="18" x极 3" y="3" rx="2" ry="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/><path d="M15 3v18"/>';
                    break;
                default:
                    bgColor = 'bg-gray-200';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/>';
        ...
        if (selectedMedia.url.match(/\.(jpg|jpeg|png|gif|svg|webp)$/i) ||
            ...
            switch(fileExtension) {
                case 'pdf':
                    bgColor = 'bg-red-100';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 极 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>';
                    break;
                case 'doc':
                case 'docx':
                    bgColor = 'bg-blue-100';
                ...
                default:
                    bgColor = 'bg-gray-200';
                    iconSvg = '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><极 d="M14 2v4a2 2 极 0 0 2 2h4"/>';
            }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = response.redirect || '{{ route('admin.teams.all') }}';
            } else {
                toastr.error(response.message || 'An error occurred');
                $('button[type="submit"]').prop('disabled', false).text('Update Team Member');
            }
        },
        error: function(xhr) {
