                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">All Product Types</span>
                        </div>
                    </li>
                </ol>
            </div>
                                    ...
                                    @forelse($types as $type)
                                        <tr>
                                            ...
                                            <td class="px-14 py-4 flex justify-end gap-x-2">
                                                <a href="{{ route('admin.product-type.edit', $type->id) }}" class="text-blue-500 hover:text-blue-700 p-1 rounded hover:bg-blue-50">
                                        ...
                                        <tr>
                                            <td colspan="2" class="px-6 py-4 text-center font-medium text-gray-700">
                                                <div class="flex flex-col items-center justify-center gap-x-4 py-2">
                                                    ...
                                                    <span class="pt-2 text-lg">No Product Types found.</span>
                                                </div>
                                                <span class="text-gray-500">Create your first product type to get started!</span>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                ...
                <div class="w-full lg:!w-1/3">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <h2 class="text-xl font-semibold mb-4 text-gray-800 border-b pb-2">
                            {{ isset($typeToEdit) ? 'Edit Product Type' : 'Add New Product Type' }}
                        </h2>

                        <form action="{{ isset($typeToEdit) ? route('admin.product-type.update', $typeToEdit->id) : route('admin.product-type.store') }}" method="POST">
                            @csrf
                            @if(isset($typeToEdit))
                                @method('PUT')
                            @endif
                            ...
                            <div class="flex justify-end mt-6">
                                @if(isset($typeToEdit))
                                    <a href="{{ route('admin.product-type.all') }}" class="px-4 py-2 border !border-gray-300 rounded-lg text-gray-700 hover:!bg-gray-50 mr-2">
                                        Cancel
                                    </a>
                                @endif
                                <button type="submit" class="px-4 py-2 !bg-teal-600 text-white rounded-lg hover:!bg-teal-700">
                                    {{ isset($typeToEdit) ? 'Update' : 'Create' }} Product Type
                                </button>
                            </div>
                        </form>
                    </div>
...
@push('scripts')
    <script>
        function confirmDelete(id) {
            ...
            form.action = `/admin/product-type/${id}`;
