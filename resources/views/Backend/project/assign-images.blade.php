...
<div class="flex-1 p-6">
    <div class="max-w-5xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('admin.project.all') }}" class="ml-1 text-lg font-medium text-gray-极0 hover:!text-teal-600 md:ml-2">Projects</a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        <svg class="w-3 h-3 mx-1 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 极 6 10">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                        </svg>
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">Assign Images to Project</span>
                    </div>
                </li>
            </ol>
            <a href="{{ route('admin.project.all') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-6">
            <h2 class="text-xl font-semibold mb-4">Assign Images to: {{ $project->title }}</h2>

            <form id="assign-images-form" action="{{ route('admin.project.store-images', $project->id) }}" method="POST">
                @csrf

                <!-- Selected Images -->
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-700 mb-3">Selected Images</h3>
                    <div id="selected-images-container" class="border-2 border-dashed border-gray-300 rounded-lg p-4 min-h-48">
                        ...
                        <div id="selected-images-list" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"></div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <button type="button" id="open-media-library" class="!bg-[#006494] hover:!bg-[#003554] text-white px-4 py-2 rounded-lg">
                            <div class="flex items-center gap-x-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-image-icon lucide-image"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><path d="m21 15-5-5-5 5"/><path d="m11 15 2-2 2 2"/></svg>
                ...
                <div class="flex justify-end space-x-3">
                    <a href="{{ route('admin.project.all') }}" class="!bg-gray-600 hover:!bg-gray-700 text-white px-6 py-2 rounded-lg">
                        Cancel
                    </a>
...
<div id="media-library-modal" class="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div id="library-tab-buttons" class="flex justify-end p-6 border-t bg-gray-50 gap-x-2 hidden">
            ...
            <button id="confirm-selection-multiple" class="!bg-teal-600 hover:!bg-teal-700 text-white px-4 py-2 rounded-lg">
                Add Selected Media
            </button>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function loadProjectImages() {
        $.ajax({
            url: '{{ route("admin.project.images", $project->id) }}',
            method: 'GET',
...
function renderSingleFilePreview(media) {
    ...
    function loadMediaLibrary(search = '') {
        ...
        let url = '{{ route("admin.media.ajax.all") }}?perPage=50';
        if (search) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.data && data.data.length > 0) {
                    ...
                    // Initialize modalSelectedMedia based on currently selectedMediaItems (so previously selected show as checked)
                    modalSelectedMedia = mediaLibraryItems.filter(item => selectedMediaItems.some(sel => sel.id === item.id));

                    renderMediaLibrary(mediaLibraryItems);

                    // ensure DOM checkboxes reflect modalSelectedMedia
                    modalSelectedMedia.forEach(item => {
                        const $el = $(`.media-item[data-media-id="${item.id}"]`);
                        $el.addClass('selected');
                        $el.find('.media-checkbox').prop('checked', true);
                    });
    ...
    // Render media library with checkboxes for multiple selection
    function renderMediaLibrary(mediaItems) {
        ...
        $.each(mediaItems, function(index, media) {
            let previewHtml = '';
            const fileExtension = (media.original_name || media.name || '').split('.').pop().toLowerCase();
            const isSelected = modalSelectedMedia.some(item => item.id === media.id);

            if (media.mime_type && media.mime_type.startsWith('image/')) {
                previewHtml = `<img src="${media.url}" alt="${media.original_name || media.name}" class="w-full h-24 object-scale-down">`;
        ...
        $mediaContent.find('.media-checkbox').off('change').on('change', function() {
            const mediaId = parseInt($(this).data('media-id'));
    ...
    function selectAllMedia() {
        modalSelectedMedia = [...mediaLibraryItems];
        $('.media-checkbox').prop('checked', true);
        $('.media-item').addClass('selected');
    }
    ...
    if (tab === 'upload') {
        ...
        // Check if we already have media items loaded
        if (mediaLibraryItems.length === 0) {
            loadMediaLibrary();
        } else {
            // If we already have media items, just render them
            renderMediaLibrary(mediaLibraryItems);
        }
    }
}
    ...
    // Optional select all button handler (if present in UI)
    $('#select-all-media').on('click', function() {
        if ($(this).text() === 'Select All') {
            selectAllMedia();
            $(this).text('Deselect All').removeClass('bg-gray-200').addClass('bg-teal-100 text-teal-800');
        } else {
            deselectAllMedia();
            $(this).text('Select All').removeClass('bg-teal-100 text-teal-800').addClass('bg-gray-200');
        }
    });
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                localStorage.setItem('toastr_success', response.message) || '{{ route('admin.project.all') }}';
