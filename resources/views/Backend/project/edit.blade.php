...
<div class="flex-1 p-6">
    <div class="max-w-5xl mx-auto">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$project->title}}</span>
                    </div>
                </li>
            </ol>
        ...
        <form id="project-form" action="{{ route('admin.project.update', $project->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($project->media)
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($project->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                    ...
                                    <div class="flex flex-col justify-center gap-2">
                                        ...
                                        <input type="hidden" id="selected-media-id" name="media_id" value="{{ old('media_id', $project->media_id) }}">
                                    </div>
                                </div>
                    ...
                    <div class="bg-white p-6 rounded-lg shadow">
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <select name="project_category_id" id="project_category_id" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
                            ...
                            <div>
                                ...
                                <select name="project_type_id" id="project_type_id" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = response.redirect || '{{ route('admin.project.all') }}';
            } else {
                toastr.error(response.message || 'An error occurred');
                $('button[type="submit"]').prop('disabled', false).text('Update Project');
            }
        },
        error: function(xhr) {
