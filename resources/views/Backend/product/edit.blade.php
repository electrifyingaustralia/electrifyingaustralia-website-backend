            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2">Edit Product</span>
                    </div>
                </li>
                <li aria-current="page" class="hidden lg:block">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-lg font-medium text-gray-500 md:ml-2 truncate lg:!max-w-[15rem]">{{$product->name}}</span>
                    </div>
                </li>
            </ol>
            <a href="{{ route('admin.product.all') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Enter product model_number"
                                />
                                @error('model_number')
                                    <p class="!text-red-600 text-sm">{{$message}}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input
                                    type="text" id="warranty" name="warranty"
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                            ...
                            <div>
                                <label for="product_link" class="block text-sm font-medium text-gray-700 mb-2">Link</label>
                                <input
                                    type="text" id="product_link" name="product_link"
                                    class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                            ...
                            <div>
                                ...
                                <div class="flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50" id="logo-preview">
                                        @if($product->media)
                                            @if($product->media->mime_type && str_starts_with($product->media->mime_type, 'image/'))
                                                <img src="{{ $product->media->url }}" alt="{{ $product->title }}" class="w-full h-full object-cover rounded-lg">
                                            ...
                                            @else
                                                @php
                                                    $fileExtension = pathinfo($product->media->original_name, PATHINFO_EXTENSION);
                                                    $bgColor = 'bg-gray-200';
                                                    $iconSvg = '';
                                    ...
                                    <div class="flex flex-col justify-center gap-2">
                                        ...
                                        <input type="hidden" id="selected-media-id" name="media_id" value="{{ old('media_id', $product->media_id) }}">
                                    </div>
                                </div>

                                <!-- Selected Media Info -->
                                <div id="selected-logo-info" class="mt-3 p-3 bg-gray-50 rounded-lg {{ $product->media ? '' : 'hidden' }}">
                                    @if($product->media)
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = response.redirect || '{{ route('admin.product.all') }}';
            } else {
