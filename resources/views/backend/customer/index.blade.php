                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <select name="type" class="w-full p-2 border !border-gray-300 rounded-lg">
                            ...
                            <option value="subscribe" {{ request('type') == 'subscribe' ? 'selected' : '' }}>Subscribe</option>
                ...
                <table class="min-w-full text-sm text-left text-gray-600">
                    <thead class="bg-gray-100 text-gray-700 uppercase text-xs">
                        <tr>
                            ...
                            <th class="px-6 py-3">Interests</th>
                            <th class="px-6 py-3">Proposal Type</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        @php
                        $quotation = $customer->quotationForms->first();
                        @endphp
                        <tr>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                @if($quotation && $quotation->interests && count($quotation->interests) > 0)
                            ...
                            <td class="px-6 py-4">
                                @if($quotation && $quotation->proposal_type)
                                <span class="font-medium text-sm">{{ $quotation->proposal_type }}</span>
                                @else
                                <span class="text-gray-400">---</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                ...
                                <div class="text-gray-500 text-xs">{{ $customer->subCategory->category ?? '' }}</div>
                            </td>
                            @php
                            $typesColors = [
                            'quotation' => 'text-blue-800 bg-blue-50',
                            'contact' => 'text-green-800 bg-green-50',
                            'service' => 'text-red-800 bg-red-50',
                            'subscribe' => 'text-slate-800 bg-slate-50',
                            'package' => 'text-purple-800 bg-purple-50',
                            ];
                            $typeColor = $typesColors[$customer->type] ?? 'text-gray-800 bg-gray-50';
                            @endphp
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-full text-xs font-medium {{ $typeColor }}">
                                    {{ ucfirst(str_replace('_', ' ', $customer->type ?? 'N/A')) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center justify-center py-8">
                                    ...
                                    <p class="mt-2 text-lg font-medium text-gray-600">No customer quotations found</p>
                                    <p class="text-gray-500">All customer quotations will appear here.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
