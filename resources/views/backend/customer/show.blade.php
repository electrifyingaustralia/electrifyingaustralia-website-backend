...
<div class="flex-1 p-3">
    <div class="max-w-6xl mx-auto">
        ...
        @if($customer->quotationForms && $customer->quotationForms->count() > 0)
        @php $quotation = $customer->quotationForms->first(); @endphp

        <div class="bg-white rounded-lg shadow mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-800">🔧 Energy Quotation Details</h2>
                <p class="text-gray-600 text-sm mt-1">Customer's energy solution requirements</p>
            </div>
            <div class="p-6">
                {{-- Interests and Proposal Type --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    @if($quotation->interests && count($quotation->interests) > 0)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Interested Solutions</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Proposal Type</label>
                        <p class="text-gray-900 font-medium">{{ $quotation->proposal_type }}</p>
                    </div>
                    @endif
                </div>

                {{-- Solar Section --}}
                @if($quotation->solar_system_size || $quotation->solar_roof_type || $quotation->solar_existing_system)
                <div class="border-t pt-4 mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">☀️ Solar Requirements</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @if($quotation->solar_existing_system)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing System</label>
                            <p class="text-gray-900">{{ $quotation->solar_existing_system }}</p>
                        </div>
                        @endif
                        @if($quotation->solar_existing_age)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing System Age</label>
                            <p class="text-gray-900">{{ $quotation->solar_existing_age }}</p>
                        </div>
                        @endif
                        @if($quotation->solar_system_size)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Desired System Size</label>
                            <p class="text-gray-900">{{ $quotation->solar_system_size }}</p>
                        </div>
                        @endif
                        @if($quotation->solar_roof_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Roof Type</label>
                            <p class="text-gray-900">{{ $quotation->solar_roof_type }}</p>
                        </div>
                        @endif
                        @if($quotation->solar_existing_size)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing System Size</label>
                            <p class="text-gray-900">{{ $quotation->solar_existing_size }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                {{-- Battery Section --}}
                @if($quotation->battery_capacity || $quotation->battery_existing || $quotation->battery_upgrade_type)
                <div class="border-t pt-4 mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">🔋 Battery Storage</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @if($quotation->battery_capacity)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Capacity</label>
                            <p class="text-gray-900">{{ $quotation->battery_capacity }}</p>
                        </div>
                        @endif
                        @if($quotation->battery_existing)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing Battery</label>
                            <p class="text-gray-900">{{ $quotation->battery_existing }}</p>
                        </div>
                        @endif
                        @if($quotation->battery_existing_age)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing Battery Age</label>
                            <p class="text-gray-900">{{ $quotation->battery_existing_age }}</p>
                        </div>
                        @endif
                        @if($quotation->battery_upgrade_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Upgrade Type</label>
                            <p class="text-gray-900">{{ $quotation->battery_upgrade_type }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                {{-- EV Charger Section --}}
                @if($quotation->ev_charger_type || $quotation->ev_charger_install_location)
                <div class="border-t pt-4 mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">🚗 EV Charger</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @if($quotation->ev_charger_existing)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Existing Charger</label>
                            <p class="text-gray-900">{{ $quotation->ev_charger_existing }}</p>
                        </div>
                        @endif
                        @if($quotation->ev_charger_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Charger Type</label>
                            <p class="text-gray-900">{{ $quotation->ev_charger_type }}</p>
                        </div>
                        @endif
                        @if($quotation->ev_charger_install_location)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Installation Location</label>
                            <p class="text-gray-900">{{ $quotation->ev_charger_install_location }}</p>
                        </div>
                        @endif
                        @if($quotation->ev_charger_upgrade_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Upgrade Type</label>
                            <p class="text-gray-900">{{ $quotation->ev_charger_upgrade_type }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                @if($quotation->phase_type || $quotation->switchboard_distance || $quotation->installation_timeframe || $quotation->property_type)
                <div class="border-t pt-4 mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">⚡ Technical Requirements</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @if($quotation->phase_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Phase Type</label>
                            <p class="text-gray-900">{{ $quotation->phase_type }}</p>
                        </div>
                        @endif
                        @if($quotation->switchboard_distance)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Switchboard Distance</label>
                            <p class="text-gray-900">{{ $quotation->switchboard_distance }}</p>
                        </div>
                        @endif
                        @if($quotation->property_type)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Property Type</label>
                            <p class="text-gray-900">{{ $quotation->property_type }}</p>
                        </div>
                        @endif
                        @if($quotation->installation_timeframe)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Installation Timeframe</label>
                            <p class="text-gray-900">{{ $quotation->installation_timeframe }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="border-t pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Average Bill Amount</label>
                            <p class="text-gray-900">${{ $quotation->bill_amount }}</p>
                        </div>
                        @if($quotation->bill_period)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Bill Period</label>
                            <p class="text-gray-900">{{ $quotation->bill_period }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="bg-gray-50 p-3 rounded-lg">
                                <p class="text-gray-700 whitespace-pre-line">
                                    {{ $service->issue_details ?? 'No details provided' }}
                                </p>
                            </div>
                        </div>
