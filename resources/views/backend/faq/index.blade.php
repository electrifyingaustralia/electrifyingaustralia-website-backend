            ...
            <div class="flex flex-col lg:!flex-row gap-6">
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-4 py-4 flex justify-end items-center gap-x-2">
                                                ...
                                                <button class="text-red-500 hover:text-red-700 p-1 rounded hover:bg-red-50"
                                                    type="button" onclick="confirmDelete({{ $faq->id }})">
                                                    <svg xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="px-6 py-4 text-center font-medium text-gray-700">
                                                ...
                                                <span class="text-gray-500">Create your first sticky header to get
                                                    started!</span>
                                            </td>
                                        </tr>
                                    @endforelse
                ...
                <div class="w-full lg:!w-1/3">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <form
                            action="{{ isset($faqToEdit) ? route('admin.faq.update', $faqToEdit->id) : route('admin.faq.store') }}"
                            method="POST">
                            @csrf
                            @if (isset($faqToEdit))
                                @method('PUT')
                            ...
                            <div class="mb-4">
                                <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Question <span
                                        class="text-red-600 font-bold">*</span></label>
                                <input type="text" id="question" name="question"
                                    value="{{ old('question', $faqToEdit->question ?? '') }}"
                            ...
                            <div class="mb-4">
                                <label for="answer" class="block text-sm font-medium text-gray-700 mb-1">Answer <span
                                        class="text-red-600 font-bold">*</span></label>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="is_active" id="is_active"
                                    class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
                                    <option value="1"
                                        {{ old('is_active', $faqToEdit->is_active ?? 1) == 1 ? 'selected' : '' }}>Active
                                    </option>
                                    <option value="0"
                                        {{ old('is_active', $faqToEdit->is_active ?? 1) == 0 ? 'selected' : '' }}>Inactive
                                    </option>
                                </select>
                            </div>

                            <div class="mb-4">
                                <label for="faq_type_id" class="block text-sm font-medium text-gray-700 mb-1">Type <span
                                        class="text-red-600 font-bold">*</span></label>
                                <select id="faq_type_id" name="faq_type_id"
                                    class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
                                    <option value="">Select Type</option>
                                    @foreach ($types as $type)
                                        <option value="{{ $type->id }}"
                                            {{ old('faq_type_id', $faqToEdit->faq_type_id ?? '') == $type->id ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
