...
@section('contents')
            ...
            <div class="flex flex-col lg:!flex-row gap-6">
                                ...
                                @foreach ($cards as $card)
                                    <div class="card bg-white rounded-xl overflow-hidden p-6">
                                        <div class="flex justify-between items-start">
                                            <div class="flex items-center">
                                                <div>
                                                    <h2 class="text-xl font-bold text-gray-800 max-w-[18rem] line-clamp-1">
                                                        {{ $card->title }}</h2>
                                                    <p class="text-gray-600 max-w-[18rem] line-clamp-3">
                                                        {{ $card->description }}</p>
                                                </div>
                                            </div>
                                            <div class="flex space-x-1">
                                                <a href="{{ route('admin.solution-card.show', $card->id) }}"
                                                    class="action-btn bg-green-100 hover:bg-green-200 text-green-600 p-2 rounded-full"
                                                    title="View">
                                                ...
                                                <a href="{{ route('admin.solution-card.edit', $card->id) }}"
                                                    class="action-btn !bg-blue-100 hover:!bg-blue-200 !text-blue-600 p-2 rounded-full"
                                                    title="Edit">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <button onclick="confirmDelete({{ $card->id }})"
                                                    class="action-btn bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded-full"
                                                    title="Delete">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="w-full lg:!w-1/3">
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <form
                            action="{{ isset($cardToEdit) ? route('admin.solution-card.update', $cardToEdit->id) : route('admin.solution-card.store') }}"
                            method="POST">
                            @csrf
                            @if (isset($cardToEdit))
                                @method('PUT')
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" id="title" name="title"
                                    value="{{ old('title', $cardToEdit->title ?? '') }}"
                            ...
                            <div class="mb-4">
                                <label for="bold_info" class="block text-sm font-medium text-gray-700 mb-1">Bold Info
                                    Content</label>
                                <input type="text" id="bold_info" name="bold_info"
                                    value="{{ old('bold_info', $cardToEdit->bold_info ?? '') }}"
                                    class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500"
                            ...
                            <div class="mb-4">
                                <label for="extra_info" class="block text-sm font-medium text-gray-700 mb-1">Extra Info
                                    Content</label>
                                <input type="text" id="extra_info" name="extra_info"
                                    value="{{ old('extra_info', $cardToEdit->extra_info ?? '') }}"
                                    class="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500"
    ...
    <div id="delete-modal" class="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <p class="text-gray-600 mb-6">Are you sure you want to delete this solution card? This action cannot be undone.
            </p>
